...
@Component({
  selector: 'app-manage-inventory-categories-modal',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './manage-inventory-categories-modal.component.html'
})
export class ManageInventoryCategoriesModalComponent {
  ...
  // Usuwanie z przeniesieniem surowców
  ...
  // Kategorie alternatywne dla przenoszenia surowców
  ...
  getItemCount(category: string): number {
    return this.inventoryService.getCategoryItemCount(category);
  }
  ...
  async onAddCategory(): Promise<void> {
    ...
    try {
      const added = await this.inventoryService.addCategory(name);
      if (added) {
  ...
  async onSaveEdit(oldCat: string): Promise<void> {
    ...
    this.isSubmitting.set(true);
    try {
      await this.inventoryService.renameCategory(oldCat, newName);
      this.cancelEdit();
      this.showFeedback(`Nazwa zmieniona z "${oldCat}" na "${newName}". Powiązane surowce zaktualizowano.`);
    } finally {
      this.isSubmitting.set(false);
    }
  }
  ...
  startDelete(cat: string): void {
    const count = this.getItemCount(cat);
    this.cancelEdit();

    this.deletingCategory.set(cat);
    if (count > 0) {
      const alternatives = this.categories().filter(c => c !== cat);
      this.targetReassignCategory.set(alternatives.length > 0 ? alternatives[0] : 'Dodatki');
    }
  }
  ...
  async confirmDeleteEmpty(cat: string): Promise<void> {
    this.isSubmitting.set(true);
    try {
      await this.inventoryService.deleteCategory(cat);
      this.cancelDelete();
  ...
  async confirmDeleteWithReassign(cat: string): Promise<void> {
    const target = this.targetReassignCategory() || 'Dodatki';
    this.isSubmitting.set(true);
    try {
      await this.inventoryService.deleteCategory(cat, target);
      this.cancelDelete();
      this.showFeedback(`Kategoria "${cat}" usunięta. Surowce przeniesiono do "${target}".`);
    } finally {
      this.isSubmitting.set(false);
    }
  }
